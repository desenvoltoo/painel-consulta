...
function App() {
  ...
  async function runQuery(event: FormEvent) { event.preventDefault(); const cleaned = selectedOption.clean(searchValue); if (!cleaned || status === 'loading') return; setStatus('loading'); setResult(null); setErrorMessage(''); try { const data = await api('/api/queries', { method: 'POST', body: JSON.stringify({ command: `${selectedOption.command} ${cleaned}` }) }); const finished = mapQuery(data); setResult(finished); setStatus(finished.status === 'FAILED' ? 'error' : 'success'); await loadHistory(); } catch (error) { setErrorMessage(error instanceof Error ? error.message : 'Falha na consulta.'); setStatus('error'); } }
  async function runBatch(event: FormEvent) { event.preventDefault(); const values = batchValues.split(/\r?\n|;/).map(item => selectedOption.clean(item)).filter(Boolean); if (!values.length || status === 'loading') return; setStatus('loading'); setErrorMessage(''); try { const data = await api('/api/queries/batch', { method: 'POST', body: JSON.stringify({ command_prefix: selectedOption.command, values }) }); const batch = mapBatch(data); setActiveBatch(batch); setBatches(previous => [batch, ...previous]); setBatchValues(''); setStatus('success'); loadBatch(batch.id).catch(() => undefined); } catch (error) { setErrorMessage(error instanceof Error ? error.message : 'Falha no lote.'); setStatus('error'); } }
  async function cancelBatch(id: string) { const data = await api(`/api/batches/${id}/cancel`, { method: 'POST' }); setActiveBatch(mapBatch(data)); loadBatch(id).catch(() => undefined); }
  async function retryBatch(id: string) { const data = await api(`/api/batches/${id}/retry-failed`, { method: 'POST' }); const batch = mapBatch(data); setActiveBatch(batch); setBatches(previous => [batch, ...previous]); loadBatch(batch.id).catch(() => undefined); }
